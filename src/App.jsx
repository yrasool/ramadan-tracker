...
function getSummaryItems(dayData) {
  if (!dayData) return [];
  const items = [];
  const presets = PRESET_ZIKR.filter(z => dayData.zikr?.[z.key]);
  if (presets.length > 0) {
    items.push({ label: "Zikr", value: presets.map(z => `${z.en} ×${dayData.zikr[z.key]}`).join(", ") });
  }
  const customs = (dayData.customZikr || []).filter(cz => cz.name);
  if (customs.length > 0) {
    items.push({ label: "Custom Zikr", value: customs.map(cz => `${cz.name}${cz.count ? ` ×${cz.count}` : ""}`).join(", ") });
  }
  if (dayData.additionalZikr) items.push({ label: "Notes", value: dayData.additionalZikr });
  if (dayData.quranPages) items.push({ label: "Quran Pages", value: dayData.quranPages });
  if (dayData.quranJuz) items.push({ label: "Juz", value: dayData.quranJuz });
  if (dayData.quranSurahRecited) items.push({ label: "Surah Recited", value: dayData.quranSurahRecited });
  if (dayData.surahsRecited) items.push({ label: "Surahs", value: dayData.surahsRecited });
  if (dayData.surahsMemorizing) items.push({ label: "Memorizing", value: dayData.surahsMemorizing });
  if (dayData.namesOfAllah) items.push({ label: "Names of Allah", value: dayData.namesOfAllah });
  return items;
}
...
.room{display:none;min-height:100vh;flex-direction:column;z-index:10;animation:roomIn .85s cubic-bezier(.22,1,.36,1) forwards;}\n.room.active{display:flex;}\n.room-enter{animation:roomPushIn .7s cubic-bezier(.22,1,.36,1) forwards;}\n@keyframes roomIn{from{opacity:0;transform:translateY(28px) scale(.97);}to{opacity:1;transform:none;}}\n@keyframes roomPushIn{from{opacity:0;transform:scale(1.05) translateY(16px);filter:blur(6px);}to{opacity:1;transform:scale(1) translateY(0);filter:blur(0);}}\n.room-topbar{display:flex;align-items:center;padding:18px 28px;border-bottom:1px solid rgba(200,169,122,.2);position:relative;background:rgba(0,0,0,.35);backdrop-filter:blur(4px);}\n.back-btn{background:transparent;border:1.5px solid var(--sand-dark);color:var(--gold);padding:9px 18px;cursor:pointer;font-family:'Cinzel Decorative',serif;font-size:12px;letter-spacing:2px;transition:all .2s;flex-shrink:0;position:relative;z-index:1;}\n.back-btn:hover{background:rgba(200,169,122,.12);border-color:var(--gold);}\n.room-title-block{text-align:center;position:absolute;left:50%;transform:translateX(-50%);pointer-events:none;}\n.rtitle-ar{font-family:'Scheherazade New',serif;font-size:clamp(28px,4.5vw,48px);display:block;color:var(--text-bright);text-shadow:0 0 18px rgba(245,200,66,.35);line-height:1.2;}\n.rtitle-en{font-family:'Cinzel Decorative',serif;font-size:clamp(12px,1.8vw,16px);letter-spacing:4px;display:block;margin-top:3px;color:var(--gold);opacity:1;}\n.rday-lbl{font-size:14px;letter-spacing:2px;display:block;margin-top:3px;color:var(--sand);opacity:1;}\n.room-content{flex:1;padding:40px 28px 80px;max-width:720px;margin:0 auto;width:100%;}\n.fsec{margin-bottom:40px;}\n.fsec-title{font-family:'Scheherazade New',serif;font-size:28px;margin-bottom:16px;display:flex;align-items:center;gap:12px;color:var(--text-bright);}\n.fsec-title::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,currentColor 0%,transparent 100%);opacity:.2;}\n.flabel{font-family:'Cinzel Decorative',serif;font-size:13px;letter-spacing:3px;text-transform:uppercase;margin-bottom:8px;display:block;color:var(--sand-pale);opacity:1;}\n.zikr-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(148px,1fr));gap:14px;}\n.zcard{border:1px solid rgba(255,255,255,.08);padding:16px 12px;position:relative;transition:border-color .2s;background:rgba(0,0,0,.3);}\n.zcard::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;}\n.zcard:focus-within{border-color:var(--sand-dark);}\n.zar{font-family:'Scheherazade New',serif;font-size:20px;display:block;margin-bottom:2px;}\n.zen{font-size:13px;opacity:.75;display:block;margin-bottom:10px;font-style:italic;}\n.zinput{background:rgba(0,0,0,.55);border:1.5px solid rgba(255,255,255,.1);color:var(--text-bright);padding:10px;font-size:24px;font-family:'Cormorant Garamond',serif;text-align:center;width:100%;outline:none;transition:all .2s;}\n.zinput:focus{border-color:var(--gold-dim);box-shadow:0 0 12px rgba(245,200,66,.18);}\n.big-input{background:rgba(0,0,0,.5);border:1.5px solid rgba(255,255,255,.1);color:var(--text-bright);padding:18px;font-size:17px;font-family:'Cormorant Garamond',serif;width:100%;outline:none;transition:all .2s;line-height:1.75;}\n.big-input:focus{border-color:var(--gold-dim);box-shadow:0 0 18px rgba(245,200,66,.14);}\ntextarea.big-input{resize:vertical;min-height:120px;}\n.input-row{display:grid;grid-template-columns:1fr 1fr;gap:16px;}\n.czwrap{display:flex;gap:10px;}\n.saving-dot{position:fixed;top:8px;right:14px;font-family:'Cinzel Decorative',serif;font-size:12px;letter-spacing:2px;color:var(--gold);opacity:.75;z-index:100;}\n.float-star{position:fixed;pointer-events:none;opacity:0;animation:starUp linear infinite;z-index:0;}\n@keyframes starUp{0%{opacity:0;transform:translateY(100vh);}8%{opacity:.45;}92%{opacity:.45;}100%{opacity:0;transform:translateY(-30px);}}\n.view-tabs{display:flex;gap:0;margin:0 0 28px;border:1.5px solid var(--sand-dark);overflow:hidden;}\n.view-tab{padding:11px 34px;border:none;cursor:pointer;font-family:'Cinzel Decorative',serif;font-size:13px;letter-spacing:2px;background:rgba(8,13,16,.9);color:#b0c8b8;transition:all .3s;border-right:1px solid var(--sand-dark);}\n.view-tab:last-child{border-right:none;}\n.view-tab.active{background:linear-gradient(135deg,#2a1800,#4a2e08);color:var(--gold);text-shadow:0 0 12px rgba(245,200,66,.6);}\n.cal-wrap{width:min(800px,94%);display:flex;flex-direction:column;gap:16px;padding-bottom:80px;}\n.cal-day{border:1px solid rgba(200,169,122,.18);background:rgba(0,0,0,.38);overflow:hidden;}\n.cal-day-hdr{padding:10px 18px;background:rgba(200,169,122,.09);border-bottom:1px solid rgba(200,169,122,.18);display:flex;align-items:center;gap:14px;}\n.cal-day-num{font-family:'Cinzel Decorative',serif;font-size:13px;letter-spacing:3px;color:var(--gold);}\n.cal-day-date{font-size:13px;color:var(--sand);letter-spacing:2px;}\n.cal-today-badge{font-family:'Cinzel Decorative',serif;font-size:10px;letter-spacing:2px;color:#1a2820;background:var(--gold);padding:2px 8px;}\n.cal-cols{display:grid;grid-template-columns:1fr 1fr;}\n.cal-col{padding:14px 18px;border-right:1px solid rgba(200,169,122,.1);}\n.cal-col:last-child{border-right:none;}\n.cal-col-name{font-family:'Scheherazade New',serif;font-size:20px;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid rgba(200,169,122,.15);display:flex;align-items:center;gap:8px;}\n.cal-col-name .cname-en{font-family:'Cinzel Decorative',serif;font-size:11px;letter-spacing:2px;opacity:.7;}\n.cal-entry{font-size:13px;color:var(--sand-pale);margin-bottom:5px;line-height:1.55;word-break:break-word;}\n.cal-entry-label{font-family:'Cinzel Decorative',serif;font-size:9px;letter-spacing:2px;color:var(--sand-dark);text-transform:uppercase;display:block;margin-bottom:1px;}\n.cal-empty{font-style:italic;color:#3a5040;font-size:13px;}\n@media(max-width:520px){.cal-cols{grid-template-columns:1fr;}.cal-col{border-right:none;border-bottom:1px solid rgba(200,169,122,.1);}.cal-col:last-child{border-bottom:none;}}\n`}</style>
      ...
      <div id="courtyard" className={courtyardClass}>
        ...
        <div className="view-tabs">
          <button
            className={`view-tab ${activeView === "track" ? "active" : ""}`}
              ...
              <div className="pc" key={u}>
                ...
                <div className="pc-track">
                  <div
                    className="pc-fill"
                    style={{
                      background: u === "Yusra" ? "linear-gradient(90deg,#8a6020,var(--gold))" : "linear-gradient(90deg,#1a6a4a,var(--zaminah))",
                      width: `${pct}%`,
                    }}
                  />
                </div>
        ...
        {activeView === "calendar" && (
          <div className="cal-wrap">
            {Array.from({ length: todayNum }, (_, i) => i + 1).reverse().map(d => {
              const yItems = getSummaryItems(getUserData(allData, "Yusra")?.[d]);
              const zItems = getSummaryItems(getUserData(allData, "Zaminah")?.[d]);
              const isToday = d === todayNum;
              return (
                <div key={d} className="cal-day">
                  <div className="cal-day-hdr">
                    <span className="cal-day-num">Day {d}</span>
                    <span className="cal-day-date">{getDayDate(d)}</span>
                    {isToday && <span className="cal-today-badge">Today</span>}
                  </div>
                  <div className="cal-cols">
                    <div className="cal-col">
                      <div className="cal-col-name" style={{ color: "var(--gold)" }}>
                        يُسرى <span className="cname-en">Yusra</span>
                      </div>
                      {yItems.length === 0
                        ? <div className="cal-empty">Nothing recorded yet</div>
                        : yItems.map((item, idx) => (
                          <div key={idx} className="cal-entry">
                            <span className="cal-entry-label">{item.label}</span>
                            {item.value}
                          </div>
                        ))
                      }
                    ...
                    <div className="cal-col">
                      <div className="cal-col-name" style={{ color: "var(--zaminah)" }}>
                        زمينة <span className="cname-en">Zaminah</span>
                      </div>
                      {zItems.length === 0
                        ? <div className="cal-empty">Nothing recorded yet</div>
                        : zItems.map((item, idx) => (
